...
public class CertificateManagerUtil {
    ...
    /**
     * Obtiene información del certificado desde el keystore
     */
    public CertificateInfo getCertificateInfo(CertificateConfig config) {
        try {
            validateCertificateConfig(config);
            
            KeyStore keyStore = KeyStore.getInstance(config.getKeyStoreType());
            keyStore.load(new java.io.FileInputStream(config.getKeyStorePath()), 
                         config.getPassword().toCharArray());
            
            // Obtener el primer alias (similar a FirstCertificateSelector)
            ...
            if (cert != null) {
                ...
                String validationMessage = "Certificado válido";
                
                try {
                    cert.checkValidity();
                } catch (Exception e) {
                    isValid = false;
                    validationMessage = "Certificado expirado o no válido: " + e.getMessage();
                }
                
                return new CertificateInfo(cert, issuer, subject, serialNumber, isValid, validationMessage);
            } else {
                return new CertificateInfo(null, "", "", "", false, "No se encontró certificado en el keystore");
            }
            
        } catch (Exception e) {
            log.error("Error obteniendo información del certificado: {}", e.getMessage());
            return new CertificateInfo(null, "", "", "", false, "Error: " + e.getMessage());
        }
    }
    
    /**
     * Verifica si el certificado es válido para firma
     */
    public boolean isCertificateValidForSigning(CertificateConfig config) {
        CertificateInfo info = getCertificateInfo(config);
        return info.isValid();
    }
    
    /**
     * Crea configuración de certificado con valores por defecto
     */
    public CertificateConfig createDefaultCertificateConfig(String keyStorePath, String password) {
        return new CertificateConfig(keyStorePath, password, "pkcs12", true);
    }
    
    /**
     * Verifica que el keystore puede ser abierto con las credenciales proporcionadas
     */
    public boolean testKeystoreAccess(CertificateConfig config) {
        try {
            validateCertificateConfig(config);
            ...
            // Verificar que tiene al menos un certificado
            return keyStore.aliases().hasMoreElements();
            
        } catch (Exception e) {
            log.error("Error accediendo al keystore: {}", e.getMessage());
            return false;
        }
    }
    
    /**
     * Obtiene el tipo de algoritmo del certificado (RSA, DSA, etc.)
     */
    public String getCertificateAlgorithm(CertificateConfig config) {
        CertificateInfo info = getCertificateInfo(config);
        if (info.isValid() && info.getCertificate() != null) {
            return info.getCertificate().getPublicKey().getAlgorithm();
        }
        return "UNKNOWN";
    }
     ...
     */
    public void logCertificateDetails(CertificateConfig config) {
        CertificateInfo info = getCertificateInfo(config);
        
        log.info("=== INFORMACIÓN DEL CERTIFICADO ===");
        log.info("Archivo: {}", config.getKeyStorePath());
        log.info("Tipo: {}", config.getKeyStoreType());
        log.info("Válido: {}", info.isValid());
        log.info("Mensaje: {}", info.getValidationMessage());
        
        if (info.isValid()) {
            log.info("Emisor: {}", info.getIssuer());
            log.info("Sujeto: {}", info.getSubject());
            log.info("Número de serie: {}", info.getSerialNumber());
            log.info("Algoritmo: {}", getCertificateAlgorithm(config));
        }
        log.info("=====================================");
    }
}